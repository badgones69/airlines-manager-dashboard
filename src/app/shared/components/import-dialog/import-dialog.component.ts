...
import { getDownloadTemplateFileInputLabel, getImportFileInputLabel, getImportButtonLabel, getNumberLabel } from '../../labels/commons/import-common';
...
export class ImportDialogComponent implements OnInit {
  ...
  checkImportFile(event: any): void {
    ...
    if (fileUploaded.name.endsWith('.txt')) {
      ...
      }).finally(() => this.importFileSize$.next(`${getNumberLabel(this.origin, this.importFileData.length)}`));
    } else {
      this.notificationService.showErrorNotification('Erreur', 'Le fichier sélectionné n\'est pas au format TXT');
    }
  }
