...
export class EditAircraftComponent implements OnInit {
  ...
  public initAircraftToEdit!: Aircraft;
  ...
  ngOnInit(): void {
    if (history.state.aircraft) {
      this.initAircraftToEdit = JSON.parse(history.state.aircraft);
      if (history.state.flights) {
        this.initAircraftToEdit.flights = history.state.flights;
      }
    }
  ...
  /* Aircraft editing */
  editAircraft(aircraft: any): void {
    aircraft.aircraftUUID = this.initAircraftToEdit.uuid;
    // Aircraft updating
    this.aircraftService.updateAircraft(aircraft).then((result: any) => {
      // If aircraft is updated
      if (result) {
