import { getUnauthorizedMessage, getForbiddenMessage, getAuthenticatedUserUneditableMessage, getTechnicalErrorTitle, getTechnicalErrorMessage } from '../../app/shared/labels/errors';
...
describe('ErrorsLabels', () => {
  ...
  it('#getUnauthorizedMessage should return unauthorized error message', () => {
    ...
    expect(unauthorizedMessage).toStrictEqual('Veuillez vous connecter pour accéder à cette page !');
  });

  it('#getForbiddenMessage should return forbidden error message', () => {
    const forbiddenMessage: string = getForbiddenMessage();
    expect(forbiddenMessage).toStrictEqual('Vous n\'êtes pas habilité(e) pour accéder à cette page !');
  });

  it('#getAuthenticatedUserUneditableMessage should return error message because authenticated user is uneditable', () => {
    const authenticatedUserUneditableMessage: string = getAuthenticatedUserUneditableMessage();
    expect(authenticatedUserUneditableMessage).toStrictEqual('Aucune action possible sur l\'utilisateur connecté !');
  });
  ...
  it('#getTechnicalErrorMessage should return technical error title', () => {
    ...
    expect(technicalErrorMessage).toStrictEqual('Une erreur est survenue : veuillez réessayer...');
  });
});