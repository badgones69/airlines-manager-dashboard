import { ADD_FORM_MODE, DELETE_FORM_MODE, EDIT_FORM_MODE } from '../../app/shared/constants/forms-constants';
import { getFormModeLabel, getFormActionLabel, getPasswordInputLabel, getSubmitButtonLabel, getSubmitButtonIcon, getResetButtonLabel, getResetButtonIcon, getResetButtonType, getRequiredFieldErrorMessage, getICAO_IATA_FieldsErrorMessage, getBlankStringFieldErrorMessage, getUnknownCountryErrorMessage } from '../../app/shared/labels/commons/form-common';
...
describe('FormCommonLabels', () => {
  ...
  it('#getICAO_IATA_FieldsErrorMessage should return ICAO/IATA fields error message', () => {
    const icao_iataFieldsErrorMessage: string = getICAO_IATA_FieldsErrorMessage();
  ...
  it('#getBlankStringFieldErrorMessage should return blank-string field error message', () => {
    const blankStringFieldsErrorMessage: string = getBlankStringFieldErrorMessage();
    expect(blankStringFieldsErrorMessage).toStrictEqual('min. 1 caractère obligatoire');
  });
