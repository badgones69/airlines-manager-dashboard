...
import { getUserFormTitle, getRepeatedPasswordInputLabel, getUserDeleteDialogMessage, getIdentityFieldsErrorMessage, getLoginFieldFormatErrorMessage, getPasswordFieldFormatErrorMessage, getRepeatedPasswordFieldFormatErrorMessage, getUserFormSuccessNotificationMessage } from '../../app/shared/labels/forms/user-form';
...
describe('UserFormLabels', () => {
  ...
  it('#getUserDeleteDialogMessage should return user delete dialog message', () => {
    ...
    expect(userDeleteDialogMessage).toStrictEqual('Confirmez-vous la suppression définitive de cet utilisateur ?');
  });
  ...
  it('#getLoginFieldFormatErrorMessage should return login field error message', () => {
    ...
    expect(loginFieldErrorMessage).toStrictEqual('format invalide (a[-b].c[-d])');
  });

  it('#getPasswordFieldFormatErrorMessage should return password field error message', () => {
    const passwordFieldErrorMessage: string = getPasswordFieldFormatErrorMessage();
    expect(passwordFieldErrorMessage).toStrictEqual('mot de passe trop court et/ou pas assez sécurisé');
  });

  it('#getRepeatedPasswordFieldFormatErrorMessage should return repeated password field error message', () => {
    const repeatedPasswordFieldErrorMessage: string = getRepeatedPasswordFieldFormatErrorMessage();
    expect(repeatedPasswordFieldErrorMessage).toStrictEqual('mots de passe différents');
  });

  it('#getUserFormSuccessNotificationMessage should return success notification message', () => {
    const successNotificationMessage: string = getUserFormSuccessNotificationMessage(ADD_FORM_MODE);
    expect(successNotificationMessage).toStrictEqual('Votre utilisateur a bien été créé(e) !');
  });
});