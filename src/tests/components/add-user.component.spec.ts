...
describe('AddUserComponent', () => {
  ...
  it('#ngOnInit should initialize "Add user" component', () => {
    ...
    const addUserComponent: AddUserComponent = new AddUserComponent(Inject(UserService), Inject(NotificationService), Inject(Router));
  ...
  it('#addUser should create user in DB', async () => {
    ...
    vi.spyOn(addUserComponent, 'addUser').mockImplementation(() => {
      mockAddUserComponent.userService.createUser(userToCreate).then(async (result) => {
        if (result.data) {
          ...
          const toastrSuccess: any = mockAddUserComponent.notificationService.showSuccessNotification(
          ...
          expect(toastrSuccess.message).toStrictEqual('Votre utilisateur a bien été créé(e) !');

          await harness.navigateByUrl('/users/list');
          expect(harness.routeNativeElement?.textContent).toBe('List users');
        } else {
          const toastrError: any = mockAddUserComponent.notificationService.showErrorNotification(
