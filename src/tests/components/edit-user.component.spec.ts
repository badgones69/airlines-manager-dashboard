...
import { getUserFormTitle, getUserFormSuccessNotificationMessage } from '../../app/shared/labels/forms/user-form';
...
describe('EditUserComponent', () => {
  ...
  it('#ngOnInit should initialize "Edit user" component', async () => {
    let mockEditUserComponent: MockEditUserComponent = new MockEditUserComponent();
    const editUserComponent: EditUserComponent = new EditUserComponent(Inject(UserService), Inject(NotificationService), Inject(ActivatedRoute), Inject(Router));
    vi.spyOn(editUserComponent, 'ngOnInit').mockImplementation(() => {
      editUserComponent.userUUID = mockEditUserComponent.route.snapshot.paramMap.get('uuid') ?? '';

      mockEditUserComponent.userService.findUser(editUserComponent.userUUID).then(async (userToEdit) => {
        editUserComponent.initUserToEdit = editUserComponent.userMapper.userFromDB(userToEdit.data);
  ...
  it('#editUser should update user in DB', async () => {
    ...
    vi.spyOn(editUserComponent, 'editUser').mockImplementation(() => {
      mockEditUserComponent.userService.updateUser(userToUpdate).then(async (result) => {
        if (result.data) {
          ...
          const toastrSuccess: any = mockEditUserComponent.notificationService.showSuccessNotification(
          ...
          expect(toastrSuccess.title).toStrictEqual('MODIFICATION D\'UN UTILISATEUR');
          expect(toastrSuccess.message).toStrictEqual('Votre utilisateur a bien été modifié(e) !');

          await harness.navigateByUrl('/users/list');
          expect(harness.routeNativeElement?.textContent).toBe('List users');
        } else {
          const toastrError: any = mockEditUserComponent.notificationService.showErrorNotification(
