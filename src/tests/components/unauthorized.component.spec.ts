...
describe('UnauthorizedComponent', () => {
  ...
  it('#ngOnInit should initialize "Unauthorized" component', () => {
    TestBed.runInInjectionContext(() => {
      const unauthorizedComponent: UnauthorizedComponent = new UnauthorizedComponent(Inject(Router));
      unauthorizedComponent.ngOnInit();
      expect(unauthorizedComponent.message).toStrictEqual('Veuillez vous connecter pour accéder à cette page !');
      expect(unauthorizedComponent.redirectionButtonLabel).toStrictEqual('Authentification');
    });
  });
  ...
  it('#goToAuthenticationForm should redirect to "Authentication" component', async () => {
    ...
    const spy = vi.spyOn(unauthorizedComponent, 'goToAuthenticationForm').mockImplementation(() => harness.routeNativeElement?.textContent);
    unauthorizedComponent.goToAuthenticationForm();
    expect(spy).toHaveBeenCalled();
    expect(unauthorizedComponent.goToAuthenticationForm()).toBe('Authentication');
  });
});